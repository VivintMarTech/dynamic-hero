        ...
        utils.waitForElement('div.paragraph-hero').then(function () {
            for (var i = 0; i < promoCopy.length; i++) {
                ...
                if( promoCopy[i].utm_content === getUtm){
                  	...
                  	//update eyebrow
                  	document.querySelector('#hero-responsive-1098576 > div > div > div.text-container > div.clearfix.text-formatted.field.field--name-field-html-eyebrow.field--type-text-long.field--label-hidden.field__item > h1 > span').innerHTML = promoCopy[i].eyebrow;
    
                    //update title
                    document.querySelector('#hero-responsive-1098576 > div > div > div.text-container > div.field.heading.field--name-field-html-heading-h1.field--type-text-long.field--label-hidden > h2 > span').innerHTML = promoCopy[i].offer;
                  
                  //update subhead
                  if (promoCopy[i].subhead !== "") {
                    document.querySelector("#hero-responsive-1098576 > div > div > div.text-container > div.clearfix.text-formatted.field.field--name-field-html-subheading.field--type-text-long.field--label-hidden.field__item > p").innerHTML = '<span class="white">' + promoCopy[i].subhead + "</span>";
                  }
